...
if(isset($_SESSION['confirm']))
{
	...
	if($what=='q')
	{
		if($act=='c')
		{
			...
			if(mysql_query("update discussion_gcg set state=2 where query_no=$qn")) echo "Confirmed";
			else echo "Try again";
		...
		{
			if(mysql_query("update discussion_gcg set state=3 where query_no=$qn")) echo "Deleted";
			else echo "Try again";
		}
	...
	else if($what=='a')
	{
		if($act=='c')
		{
			...
			if(mysql_query("update discuss_ans_gcg set state=2 where query_no=$qn and timestamp='$tim'"))
			{
				echo <<<s
				Confirmed
...
s;
		...
		{
			if(mysql_query("update discuss_ans_gcg set state=3 where query_no=$qn and timestamp='$tim'"))
			{
				echo <<<s
				Deleted
				<script type="text/javascript">
