...
<?php
$ct=htmlentities(addslashes($_GET['answers']));
include("../../config_db25788.php");
$q11=mysql_query("select distinct query_no from discuss_ans_gcg where state=1 order by query_no desc")or die(mysql_error());
while($quer_no=mysql_fetch_array($q11))
{
	$q=mysql_query("select * from discussion_gcg where post_subject='$ct' and query_no=$quer_no[0]") or die(mysql_error());
	while($r=mysql_fetch_array($q)){
		$q1=mysql_query("select * from discuss_ans_gcg where query_no=".$r['query_no']." and state=1 order by timestamp desc")or die(mysql_error());
